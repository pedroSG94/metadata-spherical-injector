...
void SA3DBox::print_box() {
    ...
    __android_log_print(ANDROID_LOG_INFO, "metaLibrary", "\t\tAmbisonic Type: %s", ambisonic_type);
    __android_log_print(ANDROID_LOG_INFO, "metaLibrary", "\t\tAmbisonic Order: %d", m_iAmbisonicOrder);
    __android_log_print(ANDROID_LOG_INFO, "metaLibrary", "\t\tAmbisonic Normalization: %s", ambisonic_normalization);
    __android_log_print(ANDROID_LOG_INFO, "metaLibrary", "\t\tNumber of Channels: %d", m_iNumChannels);
}

std::string SA3DBox::get_metadata_string() {
    // Outputs a concise single line audio metadata string.
    const char *map = mapToString().c_str();
